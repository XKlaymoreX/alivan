...
exports.newInvitation = async (req, res) => {
  try {
    ...
    const alreadyExists = await inviteSchema.find({
    ...
    if (alreadyExists.length == 0) {
      ...
      res.status(500)
        return res.send("Esiste già un invito a questo nome!")
    }
  } catch (error) {
...
exports.deleteInvitation = async (req, res) => {
  try {
    ...
    await binSchema.create({
      _id: result._id,
      familyName: result.familyName,
      presentsNumber: result.presentsNumber,
      __v: result.__v,
    });
    return res.sendStatus(200);
  } catch (error) {
